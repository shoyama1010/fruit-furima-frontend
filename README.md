...
# アプリケーションURL
- ローカル環境(フロント表示では、会員登録しないと見えないように設定中)
...
- ローカル環境ではCookie認証が動作したが、VercelとRailwayの別ドメイン構成ではセッションCookieが正しく送信されず、ログイン状態を維持できなかった
- `credentials: "include"`を付けても、本番環境では`/api/user`が401または500となり、Cookie認証とトークン認証の違いを整理する必要があった
- Seederの画像パス変更後も、Next.js側で従来の`/storage/`を付与していたため、商品一覧や詳細画面で画像が表示されなかった
- Laravel ControllerでDBファサードを使用していたにもかかわらず、importが不足しており、会員登録時にクラス未検出エラーが発生した

# 将来への改善点

- ヘッダー内のボタン配置や余白を調整し、レスポンシブ表示を改善する
- 商品検索・並び替え機能について、条件の追加やUI改善を行い、商品を探しやすくする
- 商品画像の保存・配信方法を見直し、本番環境でも安定して画像を表示できる構成に改善する
